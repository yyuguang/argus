...
const buttonSize = computed<ComponentSize | undefined>(() => {
  return props.size || (props.tableAction ? 'small' : undefined)
})

const buttonClass = computed(() => [
  `${prefixCls} color-#fff`,
  {
    [`${prefixCls}--table-action`]: props.tableAction
  }
])

const buttonProps = computed(() => {
  const { tableAction, ...rest } = props
  return rest
})
