  ...
  setup(props, { attrs, emit, slots, expose }) {
    ...
    const renderTreeTableColumn = (columnsChildren: TableColumn[]) => {
      ...
      return columnsChildren.map((v) => {
        ...
        props.className = resolveActionColumnClassName(v) || props.className
    ...
    const resolveActionColumnClassName = (column: TableColumn) => {
      const classNames = [column.className]
      if (column.field === 'action') {
        classNames.push('table-action-column')
      }
...
:deep(.table-action-column .el-button) {
  min-width: 64px;
  min-height: 24px;
  padding-inline: 10px;
  font-size: 12px;
